 ...
 * @author andres
 */
public class HeapAeronave {
    private LinkedList<Aeronave> lista;

    public HeapAeronave() {
        lista = new LinkedList<>();
    }

    public HeapAeronave(LinkedList<Aeronave> aeronaves) {
        this.lista = aeronaves;
    }

    /**
     * @return the lista
     */
    public LinkedList<Aeronave> getLista() {
        return lista;
    }
     ...
     */
    public void setLista(LinkedList<Aeronave> lista) {
        this.lista = lista;
    }
    ...
    private void exChange(int posicionA, int posicionB) {
        Aeronave auxiliar = lista.get(posicionA);
    ...
    private void minHeapify(int posicionInicial, int heapSize) {
        ...
        int shortest;
        if((left <= heapSize) && (lista.get(left).getAltura() < lista.get(posicionInicial).getAltura()))
            shortest = left;
        else
            shortest = posicionInicial;
        if((right <= heapSize) && (lista.get(right).getAltura() < lista.get(shortest).getAltura()))
            shortest = right;
    ...
    private void maxHeapify(int posicionInicial, int heapSize) {
        ...
        if((right <= heapSize) && (lista.get(right).getAltura() > lista.get(largest).getAltura()))
            largest = right;
     ...
     * @return 
     */
    public Aeronave dequeueMinimo() {
        buildMinHeap(lista.size()-1);
        Aeronave cabeza = cabeza();
        lista.removeFirst();
        return cabeza;
    }
     ...
     * @param aeronave 
     */
    public void enqueue(Aeronave aeronave) {
        lista.add(aeronave);
    }
